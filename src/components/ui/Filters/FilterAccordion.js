...
export default class Filters {
    constructor() {
        ...
        this.submitBtn = this.filters.querySelector(
            '.filter-accordion__submit'
        );

        this.filterItems = [
            ...this.filters.querySelectorAll(
                '.accordion__item:not(.accordion__item_fake)'
            ),
        ];
    ...
    init() {
        ...
        document.addEventListener('modal:reset', (e) => {
            ...
            const filterContent =
                e.target.querySelector('.my-modal__body').children;
            filterContent[0].dispatchEvent(
                new Event('filter:reset', { bubbles: true })
            );
        });
        ...
        this.filters.addEventListener('modal:afterClose', (e) => {
            ...
            const inputs = [
                ...modal.querySelectorAll('input:not(.filter__input-skip)'),
            ];
            const state = inputs
                .filter(
                    (input) =>
        ...
        this.filters.addEventListener('click', (e) => {
            ...
            if (btnResetAll) {
                this.filterItems.forEach((filter) => {
                    const filterContent = filter.querySelector(
    ...
    resizeHandler() {
        if (Filters.isMobile()) {
            this.filterItems.forEach((el) => {
                const filterContent = el.querySelector(
                    '.accordion__body-content'
                ).children[0];
                const modalBody = document.querySelector(
                    `[data-filter-name=${el.dataset.filterName}] .my-modal__body`
                );
                if (filterContent) modalBody.append(filterContent);
            });

            this.filterItems = [
                ...this.filters.querySelectorAll('.filter__modal'),
            ];
        } else {
            const modals = document.querySelectorAll(
                '.filter__modal[data-filter-name]'
            );
            modals.forEach((el) => {
                const filterContent =
                    el.querySelector('.my-modal__body').children[0];
                const accordionBody = this.filters.querySelector(
                    `[data-filter-name="${el.dataset.filterName}"] .accordion__body-content`
                );
    ...
    setLabel(filter) {
        ...
        const filterContent = filter.querySelector(
            `[data-filter-name=${filterName}] .${filterType}`
        );

        let labelValue;
        if (filterType === 'check-list')
            labelValue = GetLabels.checkboxList(filterContent);
        if (filterType === 'input-range')
            labelValue = GetLabels.inputRange(filterContent);

        const labelContainers = document.querySelectorAll(
            `[data-filter-name="${filterName}"] [data-filter-label]`
        );
    ...
    initFilterTypes() {
        ...
        const inputRangeArr = [
            ...this.filters.querySelectorAll('.input-range'),
        ];
